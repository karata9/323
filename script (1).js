...
document.addEventListener('DOMContentLoaded', () => {
            ...
            return `
                <div class="transaction-card ${officeClass}" data-id="${t.id}" style="border: 1px solid rgba(0,0,0,0.1); border-radius: 12px; margin-bottom: 10px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; padding: 10px 15px;">
                    <div class="transaction-card-info">
                        <h4 style="margin: 0 0 5px 0;">${t.sellerName} <span style="font-weight: bold; font-size: 0.9em; opacity: 0.8;">(بائع)</span> <span style="font-weight: bold; margin: 0 5px;">➜</span> ${t.buyerName} <span style="font-weight: bold; font-size: 0.9em; opacity: 0.8;">(مشتري)</span></h4>
            ...
            if (commPasswordInput.value === window.getGlobalPassword()) {
                if (pendingCommToggle) {
                    ...
                    if (t) {
                        ...
                        if (t.commissions[pendingCommToggle.type + 'Paid']) {
                            const waMsg = `تم دفع عمولة\nرقم العقار: ${t.propNumber}\nدفع عمولة الـ ${pendingCommToggle.type === 'seller' ? 'بائع' : 'مشتري'} (${pendingCommToggle.type === 'seller' ? t.sellerName : t.buyerName})`;
                            window.sendWhatsAppMessage(waMsg);
                        }
                    }
                }
    ...
    function showDetailsModal(t) {
        ...
        comprehensiveMsg += `تاريخ العقد: ${t.propDate || 'غير محدد'}\n\n`;

        comprehensiveMsg += `المبلغ الكلي للعقار: ${Number(t.propAmount).toLocaleString('en-US')} دينار\n\n`;

        comprehensiveMsg += `-- سجل الدفعات --\n`;
        comprehensiveMsg += `الواصل المبدئي (يوم العقد): ${Number(t.amountReceived).toLocaleString('en-US')} دينار\n`;
        
        if (t.paymentsLog && t.paymentsLog.length > 0) {
            t.paymentsLog.forEach(p => {
                comprehensiveMsg += `تسديد بتاريخ (${p.date}): ${Number(p.amount).toLocaleString('en-US')} دينار - ملاحظة: ${p.note || 'لا يوجد'}\n`;
            });
        }
        
        if (t.expensesLog && t.expensesLog.length > 0) {
            comprehensiveMsg += `\n-- سجل المصروفات (تخصم من البائع) --\n`;
            t.expensesLog.forEach(e => {
                comprehensiveMsg += `مصروف بتاريخ (${e.date}): ${Number(e.amount).toLocaleString('en-US')} دينار - السبب: ${e.reason}\n`;
            });
        }
        
        comprehensiveMsg += `\n-- الخلاصة --\n`;
        comprehensiveMsg += `مجموع الواصل الكلي: ${totalPaidByBuyer.toLocaleString('en-US')} دينار\n`;
        comprehensiveMsg += `الباقي من سعر العقار: ${currentPropRemaining.toLocaleString('en-US')} دينار\n`;
